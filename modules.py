...
class ContinuousPrefix(nn.Module):
    ...
    def __init__(self,
        ...
        init_idx = torch.randint(
            0, embedding_layer.weight.shape[0], 
            (prefix_len,)
        )

        self.continuous_embeds = nn.ParameterList()
        for i in init_idx:
            continuous_embed = nn.Parameter(
                torch.zeros(embedding_layer.embedding_dim)
            )
            with torch.no_grad():
...
class MLP(nn.Module):
    ...
    def __init__(self,
        ...
        linears = [
            nn.Linear(c_hidden, c_hidden) for _ in range(no_hidden_layers)
        ]
...
class ContinuousPrefixMLP(nn.Module):
    ...
    def __init__(self,
        ...
        self.continuous_embeds = nn.Parameter(
            torch.zeros((prefix_len, hidden_dim))
        )
        with torch.no_grad():
